...
这个生成器将LiteX的范围缩小到外设的选择和集成，以及创建具有MMAP/流式DMA接口的外设，
这些外设可以重新集成到外部设计(或LiteX SoC)中。
可以将其视为一个迷你版的Nios SOPC Builder/ Zynq或Microblaze子系统生成器，
它提供了重用LiteX支持的任何外设的可能性 :)
"""
...
class LiteXSoCGenerator(SoCMini):
    ...
    def __init__(self, name="litex_soc", sys_clk_freq=int(50e6), **kwargs):
        ...
        SoCCore.__init__(self, platform, clk_freq=sys_clk_freq, **kwargs)  # 初始化SoC核心

        # MMAP从接口 --------------------------------------------------------------------------------
        s_bus = {  # 选择总线类型
            "wishbone" : wishbone.Interface(data_width=32, address_width=32, addressing="word"),  # Wishbone总线
            "axi-lite" : axi.AXILiteInterface(data_width=32, address_width=32),  # AXI-Lite总线
        }[kwargs["bus_standard"]]
        ...
        wb_region = SoCRegion(origin=0x2000_0000, size=0x1000_0000, cached=True)  # 创建总线区域
