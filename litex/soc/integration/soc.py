...
def add_ip_address_constants(soc, name, ip_address):
    """为SoC添加IP地址常量"""
    soc.add_constant(name + "_IP_ADDRESS", ip_address)

def add_mac_address_constants(soc, name, mac_address):
    """为SoC添加MAC地址常量"""
    soc.add_constant(name + "_MAC_ADDRESS", mac_address)
...
class SoCRegion:
    ...
    def __str__(self):
        """返回内存区域的字符串表示"""
        return f"SoCRegion(origin={self.origin}, size={self.size}, mode={self.mode}, cached={self.cached}, linker={self.linker}, decode={self.decode})"

class SoCIORegion(SoCRegion):
    """SoC IO区域类，继承自SoCRegion"""
    pass
...
class SoCBusHandler(LiteXModule):
    ...
    def add_region(self, name, region):
        """添加一个内存区域"""
        self.reserved_regions[name] = region

    def alloc_region(self, name, size, cached=True):
        """分配一个内存区域"""
        region = SoCRegion(size=size, cached=cached)
        self.add_region(name, region)
        return region
    ...
    def __str__(self):
        """返回总线处理器的字符串表示"""
        return f"SoCBusHandler(name={self.name}, standard={self.standard}, data_width={self.data_width}, address_width={self.address_width}, timeout={self.timeout}, bursting={self.bursting}, interconnect={self.interconnect}, interconnect_register={self.interconnect_register}, reserved_regions={self.reserved_regions})"
...
class SoC(LiteXModule, SoCCoreCompat):
    ...
    def __init__(self, platform, sys_clk_freq,
        ...
        self.sys_clk_freq = int(sys_clk_freq) # 在此处转换为int以允许传递float到SoC。
        self.constants    = {}
        self.csr_regions  = {}
